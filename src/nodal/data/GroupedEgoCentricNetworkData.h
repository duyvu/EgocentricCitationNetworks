/*
 * GroupedEgoCentricNetworkData.h
 *
 *  Created on: Jan 17, 2011
 *      Author: duyvu
 */

#ifndef GROUPEDEGOCENTRICNETWORKDATA_H_
#define GROUPEDEGOCENTRICNETWORKDATA_H_

#include "DataTypes.h"

namespace ndip {

class GroupedEgoCentricNetworkData {
protected:
	// the number of nodes
	...
	VectorOfDiscreteTimeVectorOfEdgeEvents vectorOfDiscreteTimeVectorOfEdgeEvents;
...
public:
	...
	GroupedEgoCentricNetworkData();
	GroupedEgoCentricNetworkData(const ExposureTimeVectorType& _nodeJoiningTimes,
			const VectorOfDiscreteTimeVectorOfEdgeEvents& _vectorOfEdgeEvents,
			double _observationTimeStart, double _observationTimeEnd);
	virtual ~GroupedEgoCentricNetworkData();
	// start rolling the network data edge by edge
	void start();

	// roll a new edge and update nodal network statistics using cache structures
	virtual void updateNodalNetworkStatistics(const DiscreteTimeVectorOfEdgeEvents& edgeEvents) = 0;
	virtual void updateNodalNetworkStatistics(unsigned int indexOfEdgeEvents) {
		updateNodalNetworkStatistics(vectorOfDiscreteTimeVectorOfEdgeEvents[indexOfEdgeEvents]);
	}
...
#endif /* GROUPEDEGOCENTRICNETWORKDATA_H_ */
